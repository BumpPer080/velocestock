...
### สถานะการพัฒนา (อัปเดต 2025-11-03)
- เปิดใช้งานหน้า `/checkout` สำหรับ role `staff` พร้อมตัวสแกน `html5-qrcode` และฟอร์มเบิกสินค้า
- เพิ่ม endpoint `/api/checkouts/lookup` และ `/api/checkouts` (staff-only) เพื่อดึงข้อมูลสินค้าและตัดสต็อกแบบทรานแซกชัน
- บันทึกประวัติการเบิกลง `product_checkouts` ควบคู่กับ `product_activity_logs` (action `checkout`) สำหรับการตรวจสอบย้อนหลัง
- หน้า Product Activity แสดงกิจกรรมเบิกสินค้าได้ พร้อมรายละเอียดจำนวนที่เบิก คงเหลือ และหมายเหตุ
- ทดสอบฝั่ง backend/frontend ด้วย Jest + Supertest (`tests/checkouts.spec.js`) และ Vitest (`src/__tests__/Checkout.test.jsx`)
- แก้ encoding ของไฟล์ UI เป็น UTF-8 เพื่อให้ข้อความภาษาไทยแสดงถูกต้องในทุกหน้า
